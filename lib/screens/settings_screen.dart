        ...
        return StatefulBuilder(builder: (ctx, setSheet) {
          Future<void> submit() async {
            ...
            setSheet(() {
              ...
              if (currentCtrl.text.isEmpty) { currentErr = 'Wajib diisi'; valid = false; }
              if (newCtrl.text.length < 6)  { newErr = 'Minimal 6 karakter'; valid = false; }
              if (confirmCtrl.text != newCtrl.text) { confirmErr = 'Password tidak cocok'; valid = false; }
            });
            if (!valid) return;
            ...
            if (res['status'] == 200) {
              ...
              if (data['errors'] != null && data['errors']['current_password'] != null) {
                setSheet(() => currentErr = data['errors']['current_password'][0]);
              } else {
                setSheet(() => currentErr = data['message'] ?? 'Terjadi kesalahan');
              }
            }
          }
              ...
              children: [
                ...
                Align(
                  ...
                  child: TextButton(
                    ...
                    child: const Text('Lupa Password Lama?', 
                        style: TextStyle(color: Color(0xFF2B5A41), fontWeight: FontWeight.bold, fontSize: 13)),
                  ),
                ),
              ],
            ),
        ...
        return StatefulBuilder(builder: (ctx, setSheet) {
          Future<void> sendOtp() async {
            ...
            if (res['status'] == 200) {
              ...
              _showSuccess('OTP telah dikirim ke email Anda.');
            } else {
              setSheet(() => generalErr = res['data']['message'] ?? 'Gagal mengirim OTP.');
            }
          }
          ...
          Future<void> submit() async {
            ...
            bool valid = true;
            setSheet(() {
              otpErr = null; newErr = null; confirmErr = null; generalErr = null;
              if (otpCtrl.text.isEmpty) { otpErr = 'Wajib diisi'; valid = false; }
              if (newCtrl.text.length < 6)  { newErr = 'Minimal 6 karakter'; valid = false; }
              if (confirmCtrl.text != newCtrl.text) { confirmErr = 'Password tidak cocok'; valid = false; }
            });
            if (!valid) return;
            ...
            if (res['status'] == 200) {
              ...
              _showSuccess('Password berhasil direset!');
               ...
               setSheet(() => generalErr = res['data']['message'] ?? 'Terjadi kesalahan');
            }
          }
          ...
          return _buildBottomSheet(
            ...
            isLoading: isLoading,
            onSubmit: submit,
            submitLabel: isOtpSent ? 'Reset Password' : 'Kirim Kode OTP',
            content: Column(
              crossAxisAlignment: CrossAxisAlignment.stretch,
              children: [
                if (generalErr != null) ...[
                   Container(
                     ...
                     child: Text(generalErr!, style: const TextStyle(color: Colors.red, fontSize: 13)),
                   ),
                   const SizedBox(height: 16),
                ],
                if (!isOtpSent)
                  const Text(
                    'Kami akan mengirimkan 6 digit kode OTP ke email yang terdaftar untuk mereset password Anda.', 
        ...
        return StatefulBuilder(builder: (ctx, setSheet) {
          Future<void> submit() async {
            ...
            if (res['status'] == 200) {
              ...
              setSheet(() => phoneErr = res['data']['message'] ?? 'Terjadi kesalahan');
            }
          }
      ...
      appBar: AppBar(
        ...
        leading: IconButton(
          icon: Container(
            ...
            child: const Icon(Icons.arrow_back_ios_new, size: 16, color: Color(0xFF2B5A41)),
          ),
        ...
        actions: [
          TextButton(
            ...
            child: const Text('Mengerti',
                style: TextStyle(
                    color: Color(0xFF2B5A41), fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    );
